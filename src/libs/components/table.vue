<template>
    ...
    <el-form ref="form" :size="size" :inline="true" :model="form" :rules="rules" :label-width="formWidth">
      <el-row :gutter="24">
        ...
        <el-col :span="(24 / formSpan - (formData.length % (24 / formSpan))) * formSpan">
          <div class="flex flex-justify-start btns">
            <SGbutton :loading="loading" :title="$t('search')" @handleClick="search"> </SGbutton>
            <SGbutton :loading="loading" :title="$t('reset')" @handleClick="reset"> </SGbutton>
            ...
            <slot name="toolbar2" :loading="loading" />
            <slot name="toolbar3" :loading="loading" />
            <!-- <el-button v-if="exportBtnShow" :loading="loading" :size="size" type="default" @click="exportAsXLSX"><b>{{ $t('export') }}</b></el-button> -->
          </div>
        </el-col>
      </el-row>
    </el-form>
  ...
  <div
    v-if="isShowToolbar && ($slots.toolbar || exportBtnShow)"
    ref="toolbar"
    class="table-toolbar-wrapper flex flex-align-center flex-justify-between hide-in-print"
  >
    <div>
      <slot name="toolbar" />
    </div>
  ...
  <div class="table-box">
    ...
    <div v-if="!$slots.tableBody" ref="pagination" class="pagination-wrap">
      <el-pagination
        class="pagination"
...
import { exportTableAsXlsx } from '@/utils/export.js'
dayjs.extend(utc)
let timer
let resizeEvent
export default {
  ...
  methods: {
    ...
    getHeight() {
      ...
      this.height = parseInt(ph - pah - fh - th)
    },
    ...
    exportAsXLSX() {
      exportTableAsXlsx(this.$refs.table.$el)
    },
    add() {
