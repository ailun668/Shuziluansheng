<template>
  ...
  <div class="content">
    <div id="weather">
      <Table :data="data" :column="column" :formData="formData" :showIndex="true" :selectIcon="false" :border="false"
        :total="total" :formSpan="6" page background @search="search" @pageChange="pageChange"
        @pageSizeChange="pageSizeChange" @reset="reset" ref="baseTable" :exportBtnShow="exportBtnShow">
        <template #equipmentName>
          <el-select filterable v-model="menu.deviceCode" placeholder="select equip" style="width: 100%"
        ...
        <template #daterange>
          <el-date-picker clearable v-model="selectDate" type="datetimerange" range-separator="-"
            start-placeholder="StartDate" end-placeholder="EndDate" :disabledDate="disabledDateFn"
            @change="datePickerChange" />
        </template>
        <template #toolbar2>
          <SGbutton btnbg :loading="loading" class="graph" :title="isGraph ? $t('datalist') : $t('graph')"
            @handleClick="toggleGraph">
            <svg-icon iconName="iconfontgraph"></svg-icon>
          </SGbutton>
        </template>
        <template #toolbar3>
          <SGbutton btnbg :loading="loading" iconName="iconfontexport" :title="$t('export')"
            @handleClick="exportAsXLSX"> </SGbutton>
        </template>
      ...
      <el-dialog :close-on-click-modal="false" title="Export Data" v-model="dialogVisible" width="22%"
        :show-close="false">
        <div v-loading="loading">
          <el-date-picker :clearable="false" v-model="selectData" type="date" placeholder="Please select a date"
