<template>
  <div class="content">
    <div id="weather">
      <Table :data="data" :column="column" :formData="formData" :showIndex="true" :selection="false" :border="true"
        :total="total" :isShowFilter="false" page, @search="search" @pageChange="pageChange"
        @pageSizeChange="pageSizeChange" @reset="reset" size="default" background ref="baseTable" :formSpan="6"
        :exportBtnShow="exportBtnShow">
        <template #equipmentName>
          <el-select filterable v-model="deviceCode" placeholder="Select DeviceCode" style="width: 100%"
            @change="changeEquipement" :clearable="!isGraph">
            <el-option v-for="(item, inx) in menu.Equipmentsurvival" :key="item.deviceName + inx"
              :label="item.deviceName" :value="item.deviceCode" />
          </el-select>
        </template>
        <template #daterange>
          <el-date-picker v-model="selectDate" :shortcuts="shortcuts" type="datetimerange" range-separator="-"
            start-placeholder="StartDate" end-placeholder="EndDate" :disabledDate="disabledDateFn" :clearable="false" />
        </template>
        <template #toolbar2>
          <SGbutton btnbg v-show="deviceCode" :loading="loading" class="graph"
            :title="isGraph ? $t('datalist') : $t('graph')" @handleClick="toggleGraph">
            <svg-icon iconName="iconfontgraph"></svg-icon>
          </SGbutton>
        </template>
        <template #toolbar3>
          <SGbutton v-show="deviceCode" btnbg :loading="loading" iconName="iconfontexport" :title="$t('export')"
            @handleClick="exportAsXLSX">
          </SGbutton>
        </template>
        <template #options="{ row }"> <el-link :icon="Tickets" type="primary"
            @click="handleDetail(row)">View</el-link>&nbsp; </template>
